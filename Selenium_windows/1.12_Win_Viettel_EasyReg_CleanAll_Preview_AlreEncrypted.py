...
# Input Title
title_text="_Viettel_EasyReg_CleanAll_Preview_AlreEncrypted"
...
#   time.sleep(2)
    ...
    AlreadyEncrypted=driver.find_element_by_xpath("//div[2]/div[17]/div/div/label/span")
    AlreadyEncrypted=driver.find_element_by_id("useEncrypted")
    if AlreadyEncrypted.is_enabled():  # Check check box condition with find by id
        if not AlreadyEncrypted.is_selected():
            AlreadyEncrypted=driver.find_element_by_xpath("//div[2]/div[17]/div/div/label/span")
            AlreadyEncrypted.click()
            print('Checkbox AlreadyEncrypted selected');
        else:
            print('Checkbox AlreadyEncrypted already selected')
    else:
        print('Checkbox AlreadyEncrypted is disabled')
